...
describe("record31", () => {
  ...
  it("returns exactly 128 characters", () => {
    expect(record31(baseParams)).toHaveLength(128);
  });

  it("starts with '31'", () => {
    expect(record31(baseParams).slice(0, 2)).toBe("31");
  });

  it("places sequence number at positions 2-5", () => {
    expect(record31(baseParams).slice(2, 6)).toBe("0001");
  });

  it("places detail number at positions 6-9 (zero-padded)", () => {
    expect(record31(baseParams).slice(6, 10)).toBe("0001");
    expect(record31({ ...baseParams, detailNum: 5 }).slice(6, 10)).toBe("0005");
  });

  it("places bank ref at positions 10-30 (21 chars)", () => {
    const r = record31(baseParams);
    expect(r.slice(10, 31)).toHaveLength(21);
    expect(r.slice(10, 31).trimEnd()).toBe("E2E-001/TX001");
  });

  it("places txCodeType '1' at position 31", () => {
    expect(record31(baseParams)[31]).toBe("1");
  });

  it("places txCode at positions 32-39 (8 chars)", () => {
    expect(record31(baseParams).slice(32, 40)).toBe("04500001");
  });

  it("places commType at position 40", () => {
    expect(record31(baseParams)[40]).toBe("0");
    expect(record31({ ...baseParams, commType: "1" })[40]).toBe("1");
  });
  ...
  it("places entry date at positions 114-119 (6 chars)", () => {
    expect(record31(baseParams).slice(114, 120)).toBe("150624");
  });

  it("places sequence '000' at positions 120-122", () => {
    expect(record31(baseParams).slice(120, 123)).toBe("000");
  });

  it("places globalisation code '0' at position 123", () => {
    expect(record31(baseParams)[123]).toBe("0");
  });

  it("next code at position 124 is '1' when hasRecord32=true", () => {
    expect(record31({ ...baseParams, hasRecord32: true })[124]).toBe("1");
  });

  it("next code at position 124 is '0' when hasRecord32=false", () => {
    expect(record31(baseParams)[124]).toBe("0");
  });

  it("blank at position 125", () => {
    expect(record31(baseParams)[125]).toBe(" ");
  });

  it("link code '0' at position 126", () => {
    expect(record31(baseParams)[126]).toBe("0");
  });
