...
describe("record22", () => {
  it("returns exactly 128 characters", () => {
    expect(record22({ seqNum: "0001", comm: "", counterpartBic: "", hasMore: false }).raw).toHaveLength(128);
  });
  ...
  it("places sequence number at positions 2-5", () => {
    const r = record22({ seqNum: "0042", comm: "", counterpartBic: "", hasMore: false }).raw;
  ...
  it("places counterpart BIC at positions 98-108 (11 chars)", () => {
    const r = record22({ seqNum: "0001", comm: "", counterpartBic: "SNDRBEBB", hasMore: false }).raw;
    expect(r.slice(98, 109)).toBe("SNDRBEBB   ");
  });

  it("sets next code to '1' at position 125 when hasMore=true", () => {
    const r = record22({ seqNum: "0001", comm: "", counterpartBic: "", hasMore: true }).raw;
    expect(r[125]).toBe("1");
  });
