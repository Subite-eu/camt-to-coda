...
describe("record23", () => {
  ...
  it("returns exactly 128 characters", () => {
    expect(record23(baseParams)).toHaveLength(128);
  });

  it("starts with '23'", () => {
    expect(record23(baseParams).slice(0, 2)).toBe("23");
  });

  it("places sequence at positions 2-5", () => {
    expect(record23(baseParams).slice(2, 6)).toBe("0001");
  });

  it("places detail number '0000' at positions 6-9", () => {
    expect(record23(baseParams).slice(6, 10)).toBe("0000");
  });

  it("places counterpart IBAN at positions 10-43 (34 chars)", () => {
    const r = record23(baseParams);
  ...
  it("next code at position 125 is always '0'", () => {
    expect(record23(baseParams)[125]).toBe("0");
  });

  it("blank at position 126", () => {
    expect(record23(baseParams)[126]).toBe(" ");
  });

  it("link code at position 127 is '1' when needRecord3=true", () => {
    expect(record23({ ...baseParams, needRecord3: true })[127]).toBe("1");
  });

  it("link code at position 127 is '0' when needRecord3=false", () => {
    expect(record23({ ...baseParams, needRecord3: false })[127]).toBe("0");
  });
});
