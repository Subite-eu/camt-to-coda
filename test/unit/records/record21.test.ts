...
describe("record21", () => {
  it("returns exactly 128 characters", () => {
    expect(record21(baseParams)).toHaveLength(128);
  });

  it("starts with '21'", () => {
    const r = record21(baseParams);
  ...
  it("places movement sign at position 31 (1=debit)", () => {
    ...
    const r = record21({ ...baseParams, entry });
    expect(r[31]).toBe("1");
  });
  ...
  it("sets next code to '1' when hasMore=true", () => {
    const r = record21({ ...baseParams, hasMore: true });
    expect(r[125]).toBe("1");
  });
  ...
  it("sets globalisation code based on needRecord3", () => {
    const rNo = record21({ ...baseParams, needRecord3: false });
    const rYes = record21({ ...baseParams, needRecord3: true });
