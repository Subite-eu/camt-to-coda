...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="createRecord.0">
		...
		<!-- Identification number of the Belgium-based account holder: 0 + company 
			number -->
		<xsl:value-of select="cf:padLeft('', 11, ' ')" />
		<!-- <xsl:call-template name="padLeft"> <xsl:with-param name="string">TODO 
			</xsl:with-param> <xsl:with-param name="size"> 11 </xsl:with-param> </xsl:call-template> -->
		<!-- Blank -->
		<xsl:text> </xsl:text>
	...
	<xsl:template name="createRecord.2.1">
		...
		<!-- Detail number starts at 0000 and is increased by 1 for each movement 
			record -->
		...
		<!-- Globalisation code -->
		<xsl:text>1</xsl:text>
		<!-- Next code. 0 = no record 2 or 3 with record identification 2 is following, 
			1 otherwise -->
		<xsl:value-of
			select="cf:getNextcode($needRecord2.2_or_3)" />
		<!-- Blank -->
		<xsl:text> </xsl:text>
		<!-- Link code with next data record: 0 = no information record is following 
			, 1 otherwise -->
	...
	<xsl:template name="createRecord.2.2">
		...
		<xsl:value-of
			select="cf:padRight($counterPartBic, 11, ' ')" />
		<!-- Blanks -->
		...
		<!-- Purpose -->
		<xsl:value-of select="cf:padRight('', 4, ' ')" />
		<!-- Next code: 0 = no record 3 with record identification 2 is following, 
			1 otherwise -->
	...
	<xsl:template name="createRecord.2.3">
		...
		<xsl:value-of
			select="cf:padRight($counterPartIban, 34, ‘ ‘)" />
		<xsl:value-of select="$currency" />
		<!-- Counterparty’s name -->
		<xsl:value-of
			select="cf:padRight($counterPartName, 35, ‘ ‘)" />
		<!-- Communication (ctd.) -->
	...
	<xsl:template name="createRecord.8">
		...
		<!-- Old balance sign (0 = credit, 1 = debit) -->
		<xsl:value-of
			select="cf:getAmountSignCode($closingBalance)" />
		<!-- Old balance (12 pos. + 3 decimals) -->
		<xsl:value-of select="cf:formatBalance($closingBalance)" />
		<!-- Old balance date (DDMMYY) -->
		<xsl:value-of
			select="cf:dateToDdMmYy($closingBalanceDate)" />
		<!-- Blank -->
	...
	<xsl:template name="createRecord.9">
		...
		<xsl:value-of select="cf:padLeft($nbRecords, 6, '0')" /><!-- 
			TODO -->
		<!-- Debit movement Sum of the amounts in type 2 records with detail number 
			0000 -->
		<xsl:value-of select="cf:formatBalance($sumDebits)" /><!-- 
			TODO -->
		<!-- Credit movement Sum of the amounts in type 2 records with detail number 
			0000 -->
		<xsl:value-of select="cf:formatBalance($sumCredits)" /><!-- 
			TODO -->
		<!-- Blank -->
		...
		<xsl:variable name="sequence">
			<xsl:choose>
				...
				<xsl:when test="$electronicSequenceNumber != ''">
					<xsl:value-of
						select="$electronicSequenceNumber mod 1000" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
