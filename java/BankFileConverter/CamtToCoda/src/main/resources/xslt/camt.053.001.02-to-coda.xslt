...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:c="urn:iso:std:iso:20022:tech:xsd:camt.053.001.02" xmlns:cf="http://custom.functions.subite.eu/cf"
			...
			<xsl:for-each select="c:Ntry">
				...
				<xsl:variable name="rawComm">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:value-of select="cf:getComm(c:NtryDtls/c:TxDtls/c:RmtInf/c:Ustrd[text() != 'NOTPROVIDED'], $refs, c:NtryDtls/c:Btch/c:NbOfTxs)" />
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
